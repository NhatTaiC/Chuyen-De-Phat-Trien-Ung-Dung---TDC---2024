	...
	public class DAL_CaLam
	{
		...
		public bool ThemCaLam(DTO_CaLam caLam)
		{
			try
			{
				...
				if (caLam.MaCaLam != string.Empty)
				{
					// Check có calam trong DB CaLam hay chưa?
					var data = da.Db.CaLams.FirstOrDefault(dt => dt.MaCaLam == caLam.MaCaLam && dt.is_deleted == 0);

					if (data == null)
					{
						// Tạo đối tượng calam
						CaLam cl_insert = new CaLam
						{
						...
						da.Db.CaLams.InsertOnSubmit(cl_insert); // Them calam mới vào DB CaLam
					...
					{
						// Thông báo
						MessageBox.Show($"Ca làm +{caLam.MaCaLam}+ đã có trong danh sách ca làm!", "Thông báo",
		...
		public bool SuaCaLam(DTO_CaLam caLam)
		{
			try
			{
				...
				if (kh != null)
				{
					...
					// Thông báo
					MessageBox.Show($"Sửa thông tin ca làm +{caLam.MaCaLam}+ thành công!", "Thông báo",
						MessageBoxButtons.OK, MessageBoxIcon.Information);
					return true;
				}
				else
